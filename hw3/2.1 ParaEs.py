...
def calibration_photo(photo_path):
    # 设置要标定的角点个数,我这里使用的是8 × 11的棋盘
    x_nums = 8  # x方向上的角点个数
    y_nums = 11
    # 设置(生成)标定图在世界坐标中的坐标
    world_point = np.zeros((x_nums * y_nums, 3), np.float32)  # 生成x_nums*y_nums个坐标，每个坐标包含x,y,z三个元素
    world_point[:, :2] = 15 * np.mgrid[:x_nums, :y_nums].T.reshape(-1, 2)
    # mgrid[]生成包含两个二维矩阵的矩阵，每个矩阵都有x_nums列,y_nums行，我这里用的是15mm×15mm的方格，所以乘了15，以mm代表世界坐标的计量单位
    ...
    for image_path in images:
        ...
        print('________corners__________')
        """
    ...
    # 将内参保存起来
    np.savez(photo_path, mtx=mtx, dist=dist)
