...
class FewShotClassifierWithPrototypes(nn.Module):
    ...
    def forward(self, features_test, features_train, labels_train):
        ...
        if features_train.dim() == 5:
            features_train = cutils.preprocess_5D_features(features_train, self.global_pooling)
        ...
        classification_weights = cutils.average_train_features(features_train, labels_train)
        classification_weights = classification_weights.view(meta_batch_size, num_novel, -1)
        # ***********************************************************************

        if features_test.dim() == 5:
            features_test = cutils.preprocess_5D_features(features_test, self.global_pooling)
        assert features_test.dim() == 3

        classification_scores = tools.batch_cosine_fully_connected_layer(
            features_test, classification_weights.transpose(1, 2), scale=self.scale_cls
        )
